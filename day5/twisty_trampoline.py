...
def partOne(input):
    curIndex = 0
    steps = 0
    while curIndex >= 0 and curIndex < len(input):
        nextIndex = curIndex + input[curIndex]
        input[curIndex] += 1
        curIndex = nextIndex
        steps += 1
...
def partTwo(input):
    ...
    steps = 0
    while curIndex >= 0 and curIndex < len(input):
        incrementAmount = -1 if input[curIndex] >= 3 else 1
        nextIndex = curIndex + input[curIndex]
        input[curIndex] += incrementAmount
        curIndex = nextIndex
        steps += 1
...
assert(partOne(getInput("day5/partOne/test")) == 5)
partOne(getInput("day5/input"))

assert(partTwo(getInput("day5/partOne/test")) == 10)
partTwo(getInput("day5/input"))
